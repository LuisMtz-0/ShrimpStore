...
router.get('/', async (req, res) => {
  // find all products
  // Will retrieve all the data by category
  try {
    const Data = await Product.findAll({
       include: [Category, { model: Tag, through : ProductTag }],
    });
    // if everything is good respond with the data
    res.status(200).json(Data);
  } catch (err) {
    // other wise it will catch the err
...
router.get('/:id', async (req, res) => {
  ...
  // will retrieve product that references both teh category and tag model
  try {
